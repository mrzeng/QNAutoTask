...
namespace QNAutoTask.ChromeNs
{
	public abstract class ChromeConnector : Disposable
	{
		...
		public bool WaitForChromeOk(int timeoutMs = 1000)
		{
			...
			{
				if (!(rt = this._evslim.WaitHandle.WaitOne(timeoutMs)))
				{
		...
		protected ChromeConnector(int hwnd, string logMark)
		{
			...
			this.Timer = new NoReEnterTimer(ReconnectLoop, 2000, 1000);
		}
			...
			try
			{
				...
				if (!this._isDisposed)
				{
					...
					else
					{
						...
						this.ChromOp.ListenChromeDetachedTurbo(this.Detached);
						this._continuousInitSessionCount = 0;
						this.IsChromeOk = true;
						this.Timer.AddAction(this.ChromOp.VerifySessionAlive, 2000, 0);
