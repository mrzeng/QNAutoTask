...
namespace QNAutoTask.ChromeNs
{
    public class ChromeOperator : Disposable
    {
        ...
        private static ChromeSession CreateChromeSession(string sessionInfoUrl, HashSet<string> titleClues, bool isChatRecord)
        {
            ...
            var chatSessions = qnSessions.Where(k=> titleClues!=null && titleClues.Any(j=>k.Title.StartsWith(j))).Select(k=>k).ToList();
        ...
        public bool EvalForInsertJsSdk()
        {
            ...
            else
            {
                ...
                var cmd = FileEx.ReadAllText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "jssdk.js"));
                        ...
                        imsdk.invoke('im.singlemsg.GetRemoteHisMsg', 
                        {
                            ...
                            if(rt.hasOwnProperty('hasMore')){
                                ...
                                var tmsgs = msgs.map(msg =>{ return {
                                                            fromid: msg.fromid.nick,
                                                            toid: msg.toid.nick,
                                                            msg: JSON.stringify(msg.originalData),
                                                            msgid: msg.mcode.clientId,
                                                            msgtime: msg.sendTime,
                                                            svrtime: msg.sendTime
                                                        } });
                                ...
                                var tmsgs = rt.map(msg =>{ return {
                                                            fromid: msg.fromid.nick,
	                                                        toid: msg.toid.nick,
	                                                        msg: JSON.stringify(msg.msgbody),
	                                                        msgid: msg.msgid,
	                                                        msgtime: msg.msgtime,
	                                                        svrtime: msg.svrtime
                                                        } });                                
